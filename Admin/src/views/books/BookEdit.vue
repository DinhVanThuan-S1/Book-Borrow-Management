<template>
  <div class="book-edit fade-in-up">
    <!-- Page Header -->
            ...
            <div class="card-body">
              <div class="d-grid gap-2">
                ...
                >
                  ...
                  {{ isSubmitting ? "Đang cập nhật..." : "Cập nhật sách" }}
                </button>
                <router-link to="/books" class="btn btn-outline-secondary">
...
export default {
  ...
  setup() {
    ...
    const fetchBook = async () => {
      ...
      try {
        const response = await api.get(`/sach/${route.params.id}`);
        if (response.success) {
          ...
          Object.keys(form).forEach((key) => {
            if (key === "MaNXB") {
              form[key] = book.MaNXB?._id || "";
            } else if (key === "MaDM") {
              form[key] = book.MaDM?._id || "";
            } else if (key !== "BiaSach") {
              form[key] = book[key] || "";
            }
          });
          ...
          if (book.BiaSach) {
            imagePreview.value = getBookImage(book.BiaSach);
          }
        }
      } catch (error) {
    ...
    const handleSubmit = async () => {
      ...
      try {
        ...
        const response = await api.put(`/sach/${route.params.id}`, formData, {
          headers: {
